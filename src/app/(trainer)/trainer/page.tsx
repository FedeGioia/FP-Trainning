...
import { TrainerAction, TrainerEmptyState, TrainerMetricCard, TrainerPageHeader, TrainerStatusBadge, TrainerSurface } from '@/components/trainer-ui'
...
export default async function TrainerDashboardPage() {
  ...
  return (
    <div className="trainer-dashboard stack">
      <TrainerPageHeader
        className="trainer-dashboard__intro"
        eyebrow="Panel de control"
        title="Resumen operativo"
        description="Visión general del rendimiento y estado de tus alumnos hoy."
        actions={<><TrainerAction className="trainer-dashboard__header-action" href="/trainer/templates">Plantillas</TrainerAction><TrainerAction className="trainer-dashboard__header-action trainer-dashboard__header-action--primary" href="/trainer/students/new" variant="primary">+ Nuevo alumno</TrainerAction></>}
      />

      <section className="trainer-dashboard__metrics" aria-label="Resumen de operación">
          <TrainerMetricCard className="trainer-dashboard__metric">
            <span>Entrenan hoy</span>
            <strong>{dashboard.todayTrainings.length}<small>/ {dashboard.studentCount} activos</small></strong>
          </TrainerMetricCard>
          <TrainerMetricCard className="trainer-dashboard__metric trainer-dashboard__metric--attention">
      ...
      <div className="trainer-dashboard__content-grid">
        <TrainerSurface className="trainer-dashboard__work trainer-dashboard__work--primary" aria-label="Entrenamientos de hoy">
          ...
          {dashboard.todayTrainings.length > 0 ? <div className="trainer-dashboard__queue">
            {dashboard.todayTrainings.map((training) => (
              <Link className="trainer-dashboard__assignment trainer-list-row" href={`/trainer/assignments/${training.id}`} key={training.id}>
                ...
                <div className="trainer-dashboard__assignment-meta"><TrainerStatusBadge className={`trainer-dashboard__status trainer-dashboard__status--${training.status.toLowerCase()}`}>{training.status === 'COMPLETED' ? 'Completado' : training.status === 'IN_PROGRESS' ? 'En curso' : formatAssignmentDate(training.scheduledAt)}</TrainerStatusBadge><span>Ver rutina →</span></div>
              </Link>
            ))}
          </div> : <TrainerEmptyState className="trainer-dashboard__empty"><strong>No hay entrenamientos para hoy.</strong><span>Podés preparar las rutinas de los próximos días.</span></TrainerEmptyState>}
        </TrainerSurface>

        <aside className="trainer-dashboard__sidebar">
          <TrainerSurface className="trainer-dashboard__attention" aria-labelledby="attention-title">
          ...
          {dashboard.attention.length > 0 ? <div className="trainer-dashboard__attention-list">{dashboard.attention.map((student) => <Link href={`/trainer/students/${student.id}`} key={student.id} className="trainer-dashboard__attention-item trainer-list-row"><span className="trainer-dashboard__avatar" aria-hidden="true">{student.name.slice(0, 1)}</span><span><strong>{student.name}</strong>{student.reasons.map((reason) => <small key={reason.type}>{reason.label}</small>)}</span><b aria-hidden="true">→</b></Link>)}</div> : <TrainerEmptyState className="trainer-dashboard__empty trainer-dashboard__empty--compact"><span>Todos los alumnos tienen seguimiento al día.</span></TrainerEmptyState>}
          </TrainerSurface>
          <TrainerSurface className="trainer-dashboard__week-summary" aria-labelledby="week-status-title">
